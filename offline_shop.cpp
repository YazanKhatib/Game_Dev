...
int COfflineShop::Buy(LPCHARACTER ch, BYTE bPos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Kendi pazarindan item satin alamazsin!");
	...
	if (!LC_IsBrazil())
	{
		if (tch)
		{
			...
			if (bIsOverFlow)
				DBManager::instance().DirectQuery("UPDATE player%s SET money2 = money2 + %u WHERE id = %u", get_table_postfix(), dwPrice, tch->GetPlayerID());
			else
				tch->PointChange(POINT_GOLD, dwPrice, false);
		}
		else
			DBManager::instance().DirectQuery("UPDATE player%s SET money2 = money2 + %u WHERE id = %u", get_table_postfix(), dwPrice, ch->GetOfflineShopOwner()->GetOfflineShopRealOwner());
	}
...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("DELETE FROM offline_shop_item%s WHERE owner_id = %u and pos = %d", get_table_postfix(), dwVID, bPos));
	return pMsg->Get()->uiAffectedRows > 0;
}

BYTE COfflineShop::GetLeftItemCount(DWORD dwPID)
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM offline_shop_item%s WHERE owner_id = %u and status = 0", get_table_postfix(), dwPID));
	if (pMsg->Get()->uiNumRows == 0)
		return 0;
