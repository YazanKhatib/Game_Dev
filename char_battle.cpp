...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
	...
	TEMP = pkKiller->GetQuestFlag("Loot_item.on_off");
	TEMP1 = pkKiller->GetQuestFlag("Loot_item.on_off1");
	TEMP2 = pkKiller->GetQuestFlag("Loot_item.on_off2");
	TEMP3 = GetQuestFlag("Loot_item.on_off3"); // ãÇäÚ ÇáÓÑÞÉ
	
	ChatPacket(CHAT_TYPE_INFO, "Hello World!");
	int iLoot = 0;
	int Lootlvl = 0;
	if (TEMP == 1)
		iLoot = 700430;
		...
		iLoot = 700432;
	int RNG = number(1, 101);
	ChatPacket(CHAT_TYPE_INFO, "Random Number Generator: %d",RNG);

	if (!(RNG >= 50 && TEMP3 == 1)) {
		ChatPacket(CHAT_TYPE_INFO, "first loop");
		if (TEMP || TEMP1 || TEMP2) {
			ChatPacket(CHAT_TYPE_INFO, "Hello World!!!!!");
			if (iLoot == 700430) Lootlvl = 0; //Get Lootitem level
			else if (iLoot == 700431) Lootlvl = 1;
			else if (iLoot == 700432) Lootlvl = 2;

			ChatPacket(CHAT_TYPE_INFO, "Loot Level: %d", Lootlvl);

			LPITEM p;
			if (pkKiller->CountSpecifyItem(iLoot)) {

				std::vector < std::pair<BYTE, bool>   > vec_bSlots;
				ChatPacket(CHAT_TYPE_INFO, "Loot Level: %d", Lootlvl);
				if (Lootlvl == 2) { // Get Equipment 
					for (i = 0; i < WEAR_MAX_NUM; ++i) {
						ChatPacket(CHAT_TYPE_INFO, "Item Index: %d", i);
						p = GetWear(i);
						if (p && !IS_SET(p->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_PKDROP))
							vec_bSlots.push_back(std::make_pair(i, true)); //True for Equipments 
					}
				}
				for (i = 0; i < INVENTORY_MAX_NUM; ++i) {
					p = GetInventoryItem(i);
					if (p && !IS_SET(p->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_PKDROP)) {
						switch (p->GetVnum()) { //Skip small potions
						case 27001:
						case 27004:
						case 27051:
						case 27052:
						...
						if (Lootlvl == 2 && (p->GetVnum() >= 27001 && p->GetVnum() <= 27123) || (p->GetVnum() >= 39017 && p->GetVnum() <= 39021))
							continue;

						vec_bSlots.push_back(std::make_pair(i, false)); //False for Inventory
					}
				}
				ChatPacket(CHAT_TYPE_INFO, "VECTOR SIZE: %d", vec_bSlots.size());

				if (!vec_bSlots.empty()) {
					random_shuffle(vec_bSlots.begin(), vec_bSlots.end());
					bool tp = vec_bSlots[0].second;
					int iQty = vec_bSlots[0].first;
					if (tp == false) pkItem = GetInventoryItem(iQty);
					else pkItem = GetWear(iQty);
					if (!pkItem) return;
					ChatPacket(CHAT_TYPE_INFO, "TYPE : %d", (int)tp);

					if (Lootlvl == 0) { // Level 0 Potions only
						ChatPacket(CHAT_TYPE_INFO, "IN LVL 0 ");
						if (pkItem->GetVnum() >= 27001 && pkItem->GetVnum() <= 27123) {
							SyncQuickslot(QUICKSLOT_TYPE_ITEM, iQty, 255);
							pkItem->RemoveFromCharacter();
							pkKiller->AutoGiveItem(pkItem, true);
							pkKiller->SetQuestFlag("Loot_item.on_off", 0);
							pkKiller->RemoveSpecifyItem(iLoot, 1);

							if (pkItem->GetName() == NULL) return;

							ChatPacket(CHAT_TYPE_INFO, "áÞÏ ÞÇã ÇááÇÚÈ: %s ÈÓÑÞÉ ÇáÃÏÇÉ: %s ÈÇÓÊÎÏÇã ÞÝÇÒ ÇáÓÑÞÉ", pkKiller->GetName(), pkItem->GetName());
							pkKiller->ChatPacket(CHAT_TYPE_INFO, "áÞÏ ÞãÊ ÈÓÑÞÉ ÇáÃÏÇÉ: %s ãä ÇááÇÚÈ: %s ÈÇÓÊÎÏÇã ÞÝÇÒ ÇáÓÑÞÉ", pkItem->GetName(), this->GetName());
						}
						else
							pkKiller->ChatPacket(CHAT_TYPE_INFO, "áÞÏ æÞÚÊ ÇáÞÑÚÉ Úáì ÃÏÇÉ ÐÇÊ ãÓÊæì ÚÇáí :%s áßí ÊÍÕá Úáì ÃÏæÇÊ ÈãÓÊæì ÃÚáì Úáíß ÊØæíÑ ÞÝÇÒ ÇáÓÑÞÉ áãÓÊæì ÃÚáì", pkItem->GetName());

						return;
					}
					if (Lootlvl == 1) { // Level 1 Potions and Waters
						ChatPacket(CHAT_TYPE_INFO, "IN LVL 1 ");
						if ((pkItem->GetVnum() >= 27001 && pkItem->GetVnum() <= 27123) || (pkItem->GetVnum() >= 39017 && pkItem->GetVnum() >= 39021)) {
							SyncQuickslot(QUICKSLOT_TYPE_ITEM, iQty, 255);
							pkItem->RemoveFromCharacter();
							pkKiller->AutoGiveItem(pkItem, true);
							pkKiller->SetQuestFlag("Loot_item.on_off1", 0);
							pkKiller->RemoveSpecifyItem(iLoot, 1);
					...
					if (vec_bSlots.size() > 0) {
						ChatPacket(CHAT_TYPE_INFO, "IN LVL 2 ");
						SyncQuickslot(QUICKSLOT_TYPE_ITEM, iQty, 255);
						pkItem->RemoveFromCharacter();
						pkKiller->AutoGiveItem(pkItem, true);
						pkKiller->SetQuestFlag("Loot_item.on_off2", 0);
						pkKiller->RemoveSpecifyItem(iLoot, 1);
				...
				else pkKiller->ChatPacket(CHAT_TYPE_INFO, "ÚÊÇÏ ÇáÎÕã:%s áÇ íÍæí Úáì ÃÏæÇÊ", this->GetName());
			}
		}
		return;
	}
	else {
		if (CountSpecifyItem(700433) > 0) {
			pkKiller->RemoveSpecifyItem(iLoot, 1);
			RemoveSpecifyItem(700433, 1); //remove loot blocker item from player
			pkKiller->SetQuestFlag("Loot_item.on_off", 0);
			pkKiller->SetQuestFlag("Loot_item.on_off1", 0);
			pkKiller->SetQuestFlag("Loot_item.on_off2", 0);
			SetQuestFlag("Loot_item.on_off3", 0); //set quest to 0
			pkKiller->ChatPacket(CHAT_TYPE_INFO, "áÞÏ ÞÇã ÇááÇÚÈ: %s ÈÕÏ ÞÝÇÒ ÇáÓÑÞÉ ÈäÌÇÍ ÈÇÓÊÎÏÇã ÃÏÇÉ ÏÝÇÚ ÇáÓÇÑÞ", this->GetName());
			ChatPacket(CHAT_TYPE_INFO, "áÝÏ ÞãÊ ÈÕÏ ÇááÇÚÈ : %s ãä ÓÑÞÉ ÚÊÇÏß Úä ØÑíÞ ÞÝÇÒ ÇáÓÑÞÉ ", pkKiller->GetName());
		}
		return;
	}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if (isAgreedPVP || isDuel)
		{
			...
			if (pkKiller->GetPKMode() == PK_MODE_PEACE) {
				...
				ChatPacket(CHAT_TYPE_INFO, " <ãÈÇÑÒÉ> ÞÇ?áÇÚÈ ÈÞÊá?%s ÈæÖÚíÉ ÍÑ íÌ?Úáíß æÚáí?ÓæÖÚ æÖÚí  ãÓÇá?Çæ ÞÈæá ãÈÇÑÒÉ ", GetName());
				CPVPManager::instance().Decline(this, pkKiller);
				CPVPManager::instance().Decline(pkKiller, this);
				pkKiller->ChatPacket(CHAT_TYPE_INFO, " íÌ?Úáíß æÖ?æÖÚí  ãÓÇá? ");
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (pAttacker && this)
	{
		if (pAttacker->GetWear(WEAR_COSTUME_WEAPON) && !pAttacker->GetWear(WEAR_WEAPON))
			return false;
	}
