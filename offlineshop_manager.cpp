...
void COfflineShopManager::DestroyOfflineShop(LPCHARACTER ch, DWORD dwVID, bool bDestroyAll)
{
	if (dwVID == 0)
	{
		std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT mapIndex, mapIndex,channel FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
		if (pMsg->Get()->uiNumRows == 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Cevrimdisi pazarin yok..");
			return;
		}
	...
	else
	{
		...
		if (!npc)
		{
			std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT mapIndex,channel FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
			if (pMsg->Get()->uiNumRows == 0)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Cevrimdisi pazarin yok..");
				return;
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Kanalda olmalisin %d pazari kapatman icin burda ol %d .", g_bChannel, bChannel);
				return;
			}
			...
			if (bDestroyAll)
			{
				...
				DBManager::instance().DirectQuery("DELETE FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), offlineShop.dwRealOwner);
				return;
			}
		}
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Kanalda olmalisin %d pazari kapatman icin burda ol %d .", g_bChannel, npc->GetOfflineShopChannel());
			return;
		}
...
void COfflineShopManager::Giveback(LPCHARACTER ch)
{
	...
	while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		if (pItem)
		{
			...
			if (iEmptyPos < 0)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Deponda yer yok");
				return;
			}
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "Sizden %s alindi.", pItem->GetName());
			DBManager::instance().DirectQuery("DELETE FROM offline_shop_item%s WHERE owner_id = %u and pos = %d", get_table_postfix(), ch->GetPlayerID(), item.pos);
		}
	}
}
...
void COfflineShopManager::AddItem(LPCHARACTER ch, BYTE bDisplayPos, BYTE bPos, int iPrice)
{
	...
	std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
	MYSQL_ROW row = mysql_fetch_row(pmsg->Get()->pSQLResult);
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Bu item satilamaz");
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Kilitli item satilamaz!");
		return;
	}
	...
	snprintf(szInsertQuery, sizeof(szInsertQuery), "INSERT INTO offline_shop_item%s (%s) VALUES (%s)", get_table_postfix(), szColumns, szValues);
...
void COfflineShopManager::RemoveItem(LPCHARACTER ch, BYTE bPos)
{
	...
	if (!npc)
	{
		...
		if (!pItem)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Lutfen tekrar deneyin!");
			return;
		}
		...
		if (iEmptyPos < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Cevrimdisi pazarin burada mevcut degil!");
			return;
		}
		...
		DBManager::instance().DirectQuery("DELETE FROM offline_shop_item%s WHERE owner_id = %u and pos = %d", get_table_postfix(), ch->GetPlayerID(), bPos);
	...
	else
	{
		...
		if (!pItem)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Lutfen tekrer deneyin!");
			return;
		}
...
void COfflineShopManager::ChangePrice(LPCHARACTER ch, BYTE bPos, DWORD dwPrice)
{
	...
	DBManager::instance().DirectQuery("UPDATE offline_shop_item%s SET price = %u WHERE owner_id = %u and pos = %d", get_table_postfix(), dwPrice, ch->GetPlayerID(), bPos);
...
void COfflineShopManager::RefreshMoney(LPCHARACTER ch)
{
	...
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT money2 FROM player%s WHERE id = %u", get_table_postfix(), ch->GetPlayerID()));
...
void COfflineShopManager::ChangeOfflineShopTime(LPCHARACTER ch, BYTE bTime)
{
	...
	if (npc)
	{
		...
		else
		{
			...
			std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("UPDATE offline_shop_npc%s SET time = %d WHERE owner_id = %u", get_table_postfix(), iTime, ch->GetPlayerID()));
			ch->ChatPacket(CHAT_TYPE_INFO, "Zaman degisti: %u saat", iTime / 3600);
	...
	else
	{
		...
		p.bTime = bTime;
		std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT mapIndex FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
		if (pMsg->Get()->uiNumRows == 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Bu secenegi kullanamazsiniz!");
			return;
		}
...
void COfflineShopManager::WithdrawMoney(LPCHARACTER ch, DWORD dwRequiredMoney)
{
	...
	if (dwRequiredMoney >= dwCurrentMoney)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Senin yang az lutfen tekrar kontrol edin!");
		return;
	}
	...
	if (isOverFlow)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Lutfen istenilen miktarin - 1 rakam eskik girin.");
		return;
	}

	ch->PointChange(POINT_GOLD, dwRequiredMoney, false);
	ch->ChatPacket(CHAT_TYPE_INFO, "Alinan %u yang.", dwRequiredMoney);
