...
{
    return 0;	//icgoru kuresi
    //2012.05.17 ¡¾??e?i
    //0 : A¢´¡ío???¢¬¡¤I ¡íc?e
    //1 : ¡¾Y¡Æ¡©¡Æ? ?IA¡¤
    //2 : ¡¾Y¡Æ¡©¡Æ??? ?O?¢¬©©?, ¡Æ©©?I¡íoA¢§?¢§?¡© ¡íc?eA©¬
    //3 : ¡¾©©??A©¬
    //4 : ?¢§¡¤?
    int hyperlinks;
    bool colored;
...
void CInputMain::BraveRequestPetName(LPCHARACTER ch, const char* c_pData)
{
	...
	if (ch->GetGold() < 100000) {
		ch->ChatPacket(CHAT_TYPE_INFO, "[PetSystem] ihtiyaciniz olan 100.000 yang");
	}
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (buflen > 1 && *buf == '/')
	{
		...
		{
		ch->ChatPacket(CHAT_TYPE_INFO, "  åÐÇ ÇáÃãÑ ÛíÑ ãæÌæÏ  ");
		}
	...
	if ((ch->GetGMLevel() > GM_PLAYER && ch->GetGMLevel() < GM_IMPLEMENTOR) && (pinfo->type == CHAT_TYPE_TALKING || pinfo->type == CHAT_TYPE_SHOUT || pinfo->type == CHAT_TYPE_PARTY)) //abojad was here for blocking gm chat for gm and not store it in database  log chat  there is no constrint on  guild chat  made by amjada alfawal www.facebook.com//amjad alfawal (abojad) singture 
	{
		return iExtraLen; // no data base requied to set data in it amjad flag == false if gm send chat packet  under test not tested yet 
		// coding in metin2 wolves spical system payed by one card 38$ 
	}
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		const char* kingdoms[4] = {"|cFF47DA00|H|h[GameMaster]|cFFA7FFD4|H|h","|cFFff0000|H|hShinsoo|cFFA7FFD4|H|h","|cFFFFFF00|H|hChunjo|cFFA7FFD4|H|h","|cFF0080FF|H|hJinno|cFFA7FFD4|H|h"};
		...
		if (gSpecialShout == 1)
		{
			if (ch->GetGMLevel() != GM_PLAYER)
			{
				...
				{
					int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "%s %s: %s",kingdoms[0], ch->GetName(), buf);
				}
				else
				{
					...
					{
						int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "%s %s Lv.%d : %s",kingdoms[ch->GetEmpire()], ch->GetName(), my_level, buf);
					}
					else if ((MasterColorEmpire == 0) and (MasterLevelChat == 1))
					{
						int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "%s Lv.%d : %s",ch->GetName(), my_level, buf);
					}
					else
					{
			...
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					...
					*/
					...
					if (ch->GetMyShop() || ch->IsOpenSafebox() || ch->GetShopOwner() || ch->IsCubeOpen() || ch->GetOfflineShopOwner() || ch->IsAcceOpen())
...
{
	if (ch->GetArena() || ch->GetMapIndex() == 4 )
	{
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	if (ch->GetLevel() < 40)
	{
		 ch->ChatPacket(CHAT_TYPE_INFO, "|cFFff0000|H|h<Hata> Yetersiz level!");
		 return;
	}
...
#ifdef ENABLE_OFFLINE_SHOP_SYSTEM
	if (ch->GetExchange() || ch->IsOpenSafebox() || ch->GetShopOwner() || ch->GetMyShop() || ch->IsCubeOpen() || ch->GetOfflineShopOwner() || ch->IsAcceOpen())
	{
