...
void CHARACTER::SetAffectPotion(LPITEM item)
{
	...
	for (int i = 0; i < _countof(listVnums); i++)
	{
		if (item->GetVnum() == listVnums[i])
		{
			...
			ChatPacket(CHAT_TYPE_INFO, " Êã ÇÖÇÝÉ ÇíÞæäÉ  áÞÇÆãÉ ÇáÊÝÚíáÇÊ Ýí ÇáÔÑíØ ÇáÚáæí [%s]", item->GetName());		
		}
	}
}
#endif
...
{ 
  ...
  { 
/* 
* Type = 27 -> ITEM_BLEND ( íÍÐÝ ÇáÇßæÇ?ÇáÇÎÑí ÇáÊí áÇ ÊÑíÏ ÇáÝÍ?) 
*/     
    case 50821:    case 50822:    case 50823:    case 50824:    case 50825:    case 50826: 
/* 
* SubType = 7 -> USE_ABILITY_UP ( íÍÐÝ ÇáÇßæÇ?ÇáÇÎÑí ÇáÊí áÇ ÊÑíÏ ÇáÝÍ?) 
*/     
...
/* 
* SubType = 8 -> USE_AFFECT ( íÍÐÝ ÇáÇßæÇ?ÇáÇÎÑí ÇáÊí áÇ ÊÑíÏ ÇáÝÍ?) 
*/     
    ...
    case 76003:    case 76017:    case 76018: 
      ...
      int pGetTime[] = {10}; // ÊÚíí?ÊÇíã?ÇÐ?ßä?Ýí ÍÇÌÉ Çá?ÝÊÑÉ Øæíá  ÈÚ?ÊÓÌí?ÇáÏÎæá áÊÊãßä ãä ÇÓÊÎÏÇ?ÇáÈä?
       
      int pGetFlag = GetQuestFlag("item.last_time"); // ãá?questflag Íí?áå ãÌãæÚÉ ãä input_login.cpp (áÇ ÊÛíí? 
       
      const char* pGetMessage[] = {"|cFFd0ffcc|H|hÇäÊ áÇÊÓØíÚ ÇÓÊÎÏÇã |cFFc9ff00|H|h[%s] |cFFd0ffcc|H|hÝÞØ ÈÚÏ|cFFec03e2|H|h[%u] |cFFd0ffcc|H|hËæÇä ãä ÊÓÌíá ÇáÏÎæá!"}; // ÑÓÇá79018	ÎÊã ÇáÞØ ÇáÇÒÑÞ	íÍÊæí Úáì 2500 ÇÖÑÇÑ ãåÇÑÉ 15 ÓÑÚÉ ÓÍÑ 20.
       
      if (pGetFlag) // ÊåíÆ  questflag Úá?ÇáÈä?
      { 
        if (get_global_time() < pGetFlag + pGetTime[0]) // ÇáÊåíÆ  ááÍÕæá Úá? ËæÇä ?questflag  
        { 
          ChatPacket(CHAT_TYPE_INFO, pGetMessage[0], item->GetName(), pGetTime[0]);    // ÇáÑÓÇá  
          return false; // Return false ÅÐ?ßä?ÊÓÊÎÏã åÐ?ÇáÈä?ÃÓÑÚ ãä Êá?ÇáËæÇä?
        } 
      }     
    break;     
  }     
		...
		if (item2 = GetItem(DestCell)) {
			if (item2->GetVnum() == 55002) {
				if(item2->GetAttributeValue(0) > 0){
					ChatPacket(CHAT_TYPE_INFO, "Zaten iceride bir hayvan kutusu bulunuyor.");
				}
				else{
					if (!IsValidItemPosition(DestCell) || !(item2 = GetItem(DestCell)))
						return false;
					...
					if (GetNewPetSystem()->IsActivePet()) {
					ChatPacket(CHAT_TYPE_INFO, "Once diger pet'i gonder.");
						return false;
					}
...
//										return false;
                                    ...
                                    if (item2->GetType() == ITEM_WEAPON)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti adauga acest bonus pe arma!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_ARMOR)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti adauga acest bonus pe acest item!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_BELT)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti adauga acest bonus pe curea!"));
                                        return false;
                                    ...
                                    if (item2->GetSubType() == COSTUME_ACCE)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti adauga acest bonus pe esarfa!"));
                                        return false;
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti adauga un bonus pe acest obiect."));
										return false;
									}

									if (item2->AddRareAttribute() == true)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ai adaugat un bonus pe costum cu succes!"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu mai poti adauga bonusuri pe acest item!"));
									}
								}
								break;
...
//										return false;
									...
									if (item2->GetAttributeSetIndex() == -1)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti modifica bonusul acestui obiect."));
										return false;
									}

									if (item2->ChangeRareAttribute() == true)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ai schimbat bonusurile acestui costum cu succes!"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti schimba bonusul pe acest item!"));
									}
								}
								break;
							...
							case FUCKING_BRAZIL_ITEM_AUTO_HP_RECOVERY_S:
								{
									...
									if (NULL == pAffect)
									{
										...
										AddAffect(AFFECT_ITEM_BLOCK, POINT_NONE, 0, 0, 15, 0, true, false);
									}
									else
									{
  ...
  if (IS_ENABLE_ITEM(item->GetVnum()))
    {
        if (false == IS_ENABLE_ITEM_ZONE(GetMapIndex()))
        {
            ...
            ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»ç?ë??¼? ¾?½?´?´?."));
          		  return false;
        }
    }  
	...
	if (item)
	{
		...
		if (item->AddToGround(GetMapIndex(), pos))
		{
			...
			if (false == LC_IsBrazil())
			{
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¶³¾îÁø ¾ÆÀÌÅÛÀº %dºÐ ÈÄ »ç¶óÁý´Ï´Ù."), 300/60);
			}
			else
			{
