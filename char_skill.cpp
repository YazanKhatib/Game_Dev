...
if ((dwVnum == SKILL_SAMYEON || dwVnum == SKILL_PALBANG || dwVnum == SKILL_JEONGWI || dwVnum == SKILL_GEOMKYUNG || dwVnum == SKILL_TANHWAN || dwVnum == SKILL_GIGONGCHAM || dwVnum == SKILL_GYOKSAN || dwVnum == SKILL_DAEJINGAK || dwVnum == SKILL_CHUNKEON || dwVnum == SKILL_GEOMPUNG || dwVnum == SKILL_AMSEOP || dwVnum == SKILL_GUNGSIN || dwVnum == SKILL_CHARYUN || dwVnum == SKILL_EUNHYUNG || dwVnum == SKILL_SANGONG || dwVnum == SKILL_YEONSA || dwVnum == SKILL_KWANKYEOK || dwVnum == SKILL_HWAJO || dwVnum == SKILL_GYEONGGONG || dwVnum == SKILL_GIGUNG || dwVnum == SKILL_SWAERYUNG || dwVnum == SKILL_YONGKWON || dwVnum == SKILL_GWIGEOM || dwVnum == SKILL_TERROR || dwVnum == SKILL_JUMAGAP || dwVnum == SKILL_PABEOB || dwVnum == SKILL_MARYUNG || dwVnum == SKILL_HWAYEOMPOK || dwVnum == SKILL_MUYEONG || dwVnum == SKILL_MANASHILED || dwVnum == SKILL_TUSOK || dwVnum == SKILL_MAHWAN || dwVnum == SKILL_BIPABU || dwVnum == SKILL_YONGBI || dwVnum == SKILL_PAERYONG || dwVnum == SKILL_HOSIN || dwVnum == SKILL_REFLECT || dwVnum == SKILL_GICHEON || dwVnum == SKILL_NOEJEON || dwVnum == SKILL_BYEURAK || dwVnum == SKILL_CHAIN || dwVnum == SKILL_JEONGEOP || dwVnum == SKILL_KWAESOK || dwVnum == SKILL_JEUNGRYEOK || dwVnum == SKILL_CHAYEOL || dwVnum == SKILL_SALPOONG || dwVnum == SKILL_GONGDAB || dwVnum == SKILL_PASWAE || dwVnum == SKILL_JEOKRANG || dwVnum == SKILL_CHEONGRANG) && !GetWear(WEAR_WEAPON) )

{		
		ChatPacket(CHAT_TYPE_INFO, " áÇíãßäß ÇÓÊÎÏÇã ãåÇÑÊß ãä Ïæä áÈÓ ÓáÇÍ ");
...
if ((dwVnum == SKILL_SAMYEON || dwVnum == SKILL_PALBANG || dwVnum == SKILL_JEONGWI || dwVnum == SKILL_GEOMKYUNG || dwVnum == SKILL_TANHWAN || dwVnum == SKILL_GIGONGCHAM || dwVnum == SKILL_GYOKSAN || dwVnum == SKILL_DAEJINGAK || dwVnum == SKILL_CHUNKEON || dwVnum == SKILL_GEOMPUNG || dwVnum == SKILL_AMSEOP || dwVnum == SKILL_GUNGSIN || dwVnum == SKILL_CHARYUN || dwVnum == SKILL_EUNHYUNG || dwVnum == SKILL_SANGONG || dwVnum == SKILL_YEONSA || dwVnum == SKILL_KWANKYEOK || dwVnum == SKILL_HWAJO || dwVnum == SKILL_GYEONGGONG || dwVnum == SKILL_GIGUNG || dwVnum == SKILL_SWAERYUNG || dwVnum == SKILL_YONGKWON || dwVnum == SKILL_GWIGEOM || dwVnum == SKILL_TERROR || dwVnum == SKILL_JUMAGAP || dwVnum == SKILL_PABEOB || dwVnum == SKILL_MARYUNG || dwVnum == SKILL_HWAYEOMPOK || dwVnum == SKILL_MUYEONG || dwVnum == SKILL_MANASHILED || dwVnum == SKILL_TUSOK || dwVnum == SKILL_MAHWAN || dwVnum == SKILL_BIPABU || dwVnum == SKILL_YONGBI || dwVnum == SKILL_PAERYONG || dwVnum == SKILL_HOSIN || dwVnum == SKILL_REFLECT || dwVnum == SKILL_GICHEON || dwVnum == SKILL_NOEJEON || dwVnum == SKILL_BYEURAK || dwVnum == SKILL_CHAIN || dwVnum == SKILL_JEONGEOP || dwVnum == SKILL_KWAESOK || dwVnum == SKILL_JEUNGRYEOK || dwVnum == SKILL_CHAYEOL || dwVnum == SKILL_SALPOONG || dwVnum == SKILL_GONGDAB || dwVnum == SKILL_PASWAE || dwVnum == SKILL_JEOKRANG || dwVnum == SKILL_CHEONGRANG) && !GetWear(WEAR_WEAPON) && GetWear(WEAR_COSTUME_WEAPON))
{
			ChatPacket(CHAT_TYPE_INFO, "åÐå ËÛÑÉ áÇÊÚãá ãÚ ÊÍíÇÊ ÇÈæÌÇÏ ^_^  áä íÊã ÇÍÊÓÇÈ ÖÑÑ ãåÇÑÊß Úáì ÇáÚÏæ æÇäÊ ÊÓÊÚãá åÐå ËÛÑÉ :[ ÈáÇÔ ËÛÑÇÊ ÍÈíÈí ");
	...
	if (IsPolymorphed())
		...
		if (pkVictim != this && pkVictim->IsPC() && IsPC() )
		{


		int cheecker = pkVictim->GetQuestFlag("block_skillbyabojadd.on_off");
		...
		{
		int counter = pkVictim->GetQuestFlag("block_skillbyabojadd.count");
		counter = counter + 1;
		pkVictim->SetQuestFlag("block_skillbyabojadd.count",counter);
		pkVictim->ChatPacket(CHAT_TYPE_INFO, "  áÞÏ ÊãßäÊ ãä ÕÏ ÇáãåÇÑÉ  ÇáÇÚÈ  %s  ÓíÎÊÝí ÇáÎÊã ÈÚÏ %d ÚãáíÇÊ ÕÏ äÇÌÍÉ  ", GetName(),50-pkVictim->GetQuestFlag("block_skillbyabojadd.count"));

		if(pkVictim->GetQuestFlag("block_skillbyabojadd.count") >= 50)
		{
		pkVictim->SetQuestFlag("block_skillbyabojadd.on_off",0);
		pkVictim->RemoveSpecifyItem(31088, 1);	
		pkVictim->SetQuestFlag("block_skillbyabojadd.count",0);

		} 

		pkVictim->EffectPacket(SE_DUEL_5);
		ChatPacket(CHAT_TYPE_INFO, " áÞÏ Êãßä ÇáÎÕã ãä ÕÏ ÇáãåÇÑÉ ÎÇÕÉ Èß ÈÓÊÎÏÇã ÎÊã ÕÏ ãåÇÑÇÊ äÓÈÉ ÕÏ ãåÇÑÉ 2 %  ÇáÇÚÈ %s ", pkVictim->GetName());
		return false;
