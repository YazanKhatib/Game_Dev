...
struct FPurgeMonsters
{
	void operator() (LPENTITY ent)
	{
		if ( ent->IsType(ENTITY_CHARACTER) == true )
		{
			...
			if (lpChar->IsMonster() == true && !lpChar->IsPet() && !lpChar->IsNewPet() && !lpChar->IsMountSystem())
#else
			if ( lpChar->IsMonster() == true && !lpChar->IsPet()&& !lpChar->IsMountSystem())
#endif
...
#ifdef ENABLE_MOUNT_SYSTEM
#ifdef NEW_PET_SYSTEM
			if (lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsNewPet() && !lpChar->IsMountSystem())
#else
			if ( lpChar->IsNPC() == true && !lpChar->IsPet() && !lpChar->IsMountSystem())
#endif
