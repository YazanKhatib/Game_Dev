...
if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsMountSystem() && !pkChr->IsNewPet() && pkChr->GetRider() == NULL)
...
ACMD(do_state)
{
	...
	if (ch->GetOfflineShop())
		strlcat(buf, ", Çevrimdýþý Pazar", sizeof(buf));
#endif
...
ACMD (do_player_ban)
{
    ...
    if (!tch)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, "hedef bulamiyorum!");
        return;
    }
     
    if (ch)
        ch->ChatPacket(CHAT_TYPE_INFO, "yasak talep.");
 
    if (tch && ch != tch)
    {
        if(tch->GetGMLevel() == GM_IMPLEMENTOR)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Yasak degil/unban Yoetici!");
        } else {
...
ACMD(do_stat_plus_amount)
{
	...
	switch (subcmd)
	{
		case POINT_HT : // Ã¼·Â vagabond statü ayarlarý cmd_general.cpp 651 - char.cpp 5904
