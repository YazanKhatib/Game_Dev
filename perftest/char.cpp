...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	if (IsRidingMount())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Montaj bir d?kan a?ak i?n aþaðý."));
		return;
	}	
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "You can't sell this item because is binded!");
				return;
			}
#endif
		}
...
void CHARACTER::OpenMyOfflineShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, BYTE bTime)
{
	...
	// Verifica se o gold está acima do limite
	int64_t nTotalMoney = 0;
#ifdef ENABLE_CHEQUE_SYSTEM
	int64_t nTotalCheque = 0;
#endif
	...
	// Verifica Staff do jogo não pode abrir loja offline
	{
		if (!test_server)
		{
			if (IsGM())
			{
				ChatPacket(CHAT_TYPE_INFO, "Oyun yöneticileri Çevrimdýþý Pazar kuramaz!");
				return;
			}
		}
	}
	...
	// Verifica se o jogador tem itens atuais no banco de dados.
	{
		std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM offline_shop_item%s WHERE owner_id = %u and status = 1", get_table_postfix(), GetPlayerID()));
		MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
		...
		if (bResult)
		{
			ChatPacket(CHAT_TYPE_INFO, "Çevrimdýþý Pazar açmadan önce, eski deposundan öðeleri geri al.");
	...
	// Verifica se o jogador possui uma loja offline aberta ou não
	{
		std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), GetPlayerID()));
		MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
		...
		if (bResult)
		{
			ChatPacket(CHAT_TYPE_INFO, "Zaten pazarýn açýk. Baþka pazar kuramazsýn!");
			return;
		}
	}
	// Fim da verificação
	
	// Verifica se o mapa permite abrir uma loja offline
	if (g_bOfflineShopMapAllowLimit)
	{
		if (!offlineshop_map_allow_find(GetMapIndex()))
		{
			ChatPacket(CHAT_TYPE_INFO, "Bu haritada özel bir dükkan açamazsýn!");
			return;
		}
	}
	// Fim da verificação

	// Verifica se o jogador possui o item necessário para abrir a loja offline
	if (g_bNeedItem)
	{
		...
		if (bCount < g_bItemCount)
		{
			...
			ChatPacket(CHAT_TYPE_INFO, "Maðaza açman için. x%d %s gereklidir!", g_bItemCount, item->GetName());
			return;
		}
	}
	// Fim da verificação

	// Verifica se o jogador possui gold suficiente para abrir a loja offline
	if (g_bNeedMoney)
	{
		if (GetGold() < static_cast<int>(g_dwNeedMoney))
		{
			ChatPacket(CHAT_TYPE_INFO, "Yang yeterli deðil! [Gerekli% u]", g_dwNeedMoney);
			return;
		}
	}
	// Fim da verificação
	...
	if (!npc)
	{
		ChatPacket(CHAT_TYPE_INFO, "Maðaza oluþturulamadý. Lütfen tekrar deneyin.");
		return;
	}
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		if (cont.find((pTable + i)->pos) != cont.end())
		{
			sys_err("MY_OFFLINE_SHOP: Item Shop duplicado detectado! (name: %s)", GetName());
			return;
		}

		// Verificação ANTI_GIVE, ANTI_MYSHOP
		LPITEM pkItem = GetItem((pTable + i)->pos);
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "Bu öðe satýlamaz!");
				return;
			}
			...
			if (true == pkItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, "Ruha Baðlý öðeleri satamazsýnýz!");
				return;
			}
			...
			int iUpdateLen = snprintf(szValues, sizeof(szValues), "%u,%u,%d,%u,%u,%u,%u", pkItem->GetID(), GetPlayerID(), (pTable + i)->display_pos, pkItem->GetCount(), (pTable + i)->price, (pTable + i)->price_cheque, pkItem->GetVnum());
	...
	// Se o tempo não é igual ao tempo ilimitado, inicie o tempo!
	if (bTime != 4)
	{
	...
	char szQuery2[512];
	snprintf(szQuery2, sizeof(szQuery2), "INSERT INTO offline_shop_npc%s (owner_id, sign, name, time, x, y, z, mapIndex, channel) VALUES (%u, '%s', '%s', %d, %ld, %ld, %ld, %ld, %d)", get_table_postfix(), GetPlayerID(), c_pszSign, GetName(), iTime, GetX(), GetY(), GetZ(), GetMapIndex(), g_bChannel);
	DBManager::Instance().DirectQuery(szQuery2);
	
	// Verifica se é possivel criar a loja offline
	LPOFFLINESHOP pkOfflineShop = COfflineShopManager::instance().CreateOfflineShop(npc, GetPlayerID());
	
	if (!pkOfflineShop)
	{
		ChatPacket(CHAT_TYPE_INFO, "Maðaza açýk deðil tekrar deneyin.");
		return;
	}
	...
	// Envia um sinal da loja offline para todos
	TPacketGCShopSign p;
	p.bHeader = HEADER_GC_OFFLINE_SHOP_SIGN;
	p.dwVID = npc->GetVID();
	strlcpy(p.szSign, c_pszSign, sizeof(p.szSign));
	...
	if (bTime != 4)
		ChatPacket(CHAT_TYPE_INFO, "<Çevrimdýþý Pazar> %d seçtiðin saate göre ayarlandý!", bTime);
	else
		ChatPacket(CHAT_TYPE_INFO, "<Çevrimdýþý Pazar> Zaman sýnýrsýz olarak ayarlanmýþtýr!");
	...
	// {
		...
		// if (!item)
		// {
			// ChatPacket(CHAT_TYPE_INFO, "Ops! Algo vai mal!");
			// return;
		// }
...
EVENTFUNC(offline_shop_update_event)
{
	...
	if (remain_time <= 0)
	{
		...
		if (pChar)
		{
			pChar->ChatPacket(CHAT_TYPE_INFO, "Maðazanýn özel süresi bitti.Öðeleri yeniden envanterine aktarýldý.");			
...
//	Sistem Viewer Equipment develop by: Edson.B.Junior
bool CHARACTER::SendEquipment(LPCHARACTER tch)
{
	if (!tch)
	{
		return false;
	...
	tch->ChatPacket(CHAT_TYPE_COMMAND, "ViewEquipRequest %u", (DWORD) GetVID());
	return true;
}

void CHARACTER::SendEquipmentAccept(LPCHARACTER ch)
{
...
void CHARACTER::SendEquipmentDeny(LPCHARACTER ch)
{
	ch->ChatPacket(CHAT_TYPE_COMMAND, "ViewEquipRequestDenied %u", (DWORD) GetVID());
}
//	Sistem Viewer Equipment develop by: Edson.B.Junior
