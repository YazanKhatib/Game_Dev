...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, BYTE bNpcType)
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
		nTotalCheque += static_cast<int16_t>((pTable+n)->cheque_price);
#endif
	}
...
#endif	// END_OF_MYSHOP_PRICE_LIST
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			// MYSHOP_PRICE_LIST
#ifdef ENABLE_CHEQUE_SYSTEM
			itemkind[pkItem->GetVnum()] = TItemPriceType((pTable + i)->price / pkItem->GetCount(), (pTable + i)->cheque_price);
#else
...
#ifdef ENABLE_CHEQUE_SYSTEM
			info.price.byChequePrice = it->second.byChequePrice;
#endif
		}
...
#ifdef NEW_PET_SYSTEM
	if (IsPet() || IsNewPet() || IsMountSystem())
#else
		...
		case POINT_LEVEL_STEP:
			if (amount > 0)
			{
				...
				switch (val)
				{
					...
					case 2:
					case 3:
						if (GetLevel() < 106) PointChange(POINT_STAT, 1);
						break;
...
sys_log(0, "Check the functionality of SetNextStatePulse %d", m_dwNextStatePulse); //new system log.
	if (iNextPulse < 10)
		MonsterLog("�������·ξ����");
}
...
void CHARACTER::OpenMyOfflineShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, BYTE bTime)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("��n��O��i���� A����i�Ƣ� ����CO��E ��oA�� AI����A����I ��oA��A�� ���� ��o ������A��I��U."));
		return;
	}
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (pkChr->IsState(pkChr->m_stateMove) &&
		...
		false == IsPet() && false == IsNewPet()&& false == IsMountSystem())
#else
	if (pkChr->IsState(pkChr->m_stateMove) &&
		GetMobBattleType() != BATTLE_TYPE_RANGE &&
		GetMobBattleType() != BATTLE_TYPE_MAGIC &&
...
{
	#if defined(ENABLE_WOLFMAN_CHARACTER) && !defined(__ANTI_RESIST_MAGIC_BONUS__)
	const int METIN_STONE_NUM = 16;
#elif defined(ENABLE_WOLFMAN_CHARACTER) && defined(__ANTI_RESIST_MAGIC_BONUS__)
...
void CHARACTER::AddClMaterial(TItemPos tPos, BYTE bPos)
{
	...
	if (bPos == 1)
	{
		...
		else if (pkItemMaterial[0]->GetSize() != pkItem->GetSize())
			bStop = true;
