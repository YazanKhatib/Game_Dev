	...
	struct FKillSectree2
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet()&& !ch->IsMountSystem())
#else
				if (!ch->IsPC() && !ch->IsPet()&& !ch->IsMountSystem()))
#endif
...
#ifdef NEW_PET_SYSTEM
				if (!pChar->IsPet() && !pChar->IsNewPet() && !pChar->IsMountSystem()  && (true == pChar->IsMonster() || true == pChar->IsStone()))
#else				
