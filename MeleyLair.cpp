...
namespace MeleyLair
{
	...
	EVENTFUNC(r_meleyeffect_event)
	{
		...
		else if (bStep == 2)
		{
			if (bEffectStep == 1)
			{
				#ifdef __LASER_EFFECT_ON_50HP__
				time_t timeNow = static_cast<time_t>(get_dword_time());
	...
	void CMgrMap::SetDungeonStep(BYTE bStep)
	{
		...
		else if (bStep == 4)
		{
			...
			char szBuf[512];
			snprintf(szBuf, sizeof(szBuf), " áÏíß %d ËÇäíå áÊÏãíÑ ÇáÊãËÇá .. ÇÓÑÚ ", TIME_LIMIT_TO_KILL_STATUE);
			FNotice f(szBuf);
	...
	void CMgrMap::StartDungeon(LPCHARACTER pkChar)
	{
		...
		snprintf(szBuf1, sizeof(szBuf1), " áÞÏ ÈÏÃ ÇáÞÊÇá ÖÏ %s ", pkBoss->GetName());
		snprintf(szBuf2, sizeof(szBuf2), " áÞÏ ÝÊÍÊ ÇÈæÇÈ ÇáßåÝ ¡ ÇãÇãß %d ÓÇÚå ", (int)(TIME_LIMIT_DUNGEON / 3600));
		pkGuild->Chat(szBuf1);
		pkGuild->Chat(szBuf2);
	...
	void CMgrMap::EndDungeon(bool bSuccess, bool bGiveBack)
	{
		...
		if (pkGuild)
		{
			...
			if (bGiveBack)
			{
				if (bSuccess)
				{
					...
					if (iM > 0)
						...
						time.append(boost::lexical_cast<std::string>(iM) + " ÏÞíÞå ");
					...
					if (iS > 0)
					{
						...
						time.append(boost::lexical_cast<std::string>(iS) + " ËÇäíå ");
					}
					...
					snprintf(szBuf1, sizeof(szBuf1), " áåÐíãå ãíáí ");
					snprintf(szBuf2, sizeof(szBuf2), " ÇáæÞÊ ÇáÇÒã %s ", time.c_str());
				}
				else
				{
					iWarpTime = 10;
					iReturn = (int)(LADDER_POINTS_COST);
					snprintf(szBuf1, sizeof(szBuf1), " áÇ íãßä ÇáÈÏÃ ÇáÇä .. ãä ÝÖáß ÇÓÊÏÚí ÇÚÖÇÁ ÇáãÌãæÚå ");
					snprintf(szBuf2, sizeof(szBuf2), " ÓíÊã ÇÑÌÇÚ äÞÇØ ÇáÑÇÈØå %d ", iReturn);
				}
			}
			else
			{
				...
				snprintf(szBuf1, sizeof(szBuf1), " æÞÊ ÇáÞÊÇá %s áÞÏ ÇäÊåí !!.", pkBoss->GetName());
			...
			if (pkSectreeMap)
			{
				char szBuf[512];
				if (iWarpTime == 60)
					snprintf(szBuf, sizeof(szBuf), " áÓáÇãÊß ¡ ÓíÊã äÝáß ÎÇÑÌÇ ÈÚÏ %d ËÇäíå ", iWarpTime);
				else
					snprintf(szBuf, sizeof(szBuf), " ÌãíÚ ÇáÇÚÖÇÁ ÓíÊã äÞáåã ÎÇÑÌÇ ÈÚÏ %d ËÇäíå ", iWarpTime);
