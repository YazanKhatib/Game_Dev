...
class SECTREE
{
	public:
		...
		template <class _Func> void ForEachAround(_Func & func)
		{
			...
			/*	SECTREEID id = sectree->GetID();
				SECTREEID tid = this->GetID();

				if (id.coord.x == tid.coord.x && id.coord.y == tid.coord.y){
					sys_log(0,"this is test victim");
					m_neighbor_list.erase(it);
					continue;
				}
			*/
		...
		template <class _Func> void ForEachAround1(_Func & func)
		{
			// <Factor> Using snapshot copy to avoid side-effects
			FCollectEntity collector;
			LPSECTREE_LIST::iterator it = m_neighbor_list.begin();
