...
namespace quest 
{
	...
	int pc_set_abojadl8b(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 1;
	...
	int pc_get_abojadl8b(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
		{
		...
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPrestigeLevel());
...
#ifdef __PRESTIGE_SYSTEM__
			{ "get_abojadl8b",	pc_get_abojadl8b},
			{ "set_abojadl8b",	pc_set_abojadl8b},
#endif
