...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, SUM(points) as spoints FROM okay_event%s GROUP BY pid ORDER BY spoints DESC LIMIT 10", get_table_postfix()));
...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, points FROM okay_event%s ORDER BY points DESC LIMIT 10", get_table_postfix()));
	if (pMsg->Get()->uiNumRows == 0)
	{
		return;
	}
