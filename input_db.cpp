...
void CInputDB::Boot(const char* data)
{
	...
	// request blocked_country_ip
	{
        ...
        {
            MYSQL_ROW row;
            std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT * FROM offline_shop_npc%s", get_table_postfix()));
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	if (ch->GetShopOwner() || ch->GetExchange() || ch->GetMyShop() || ch->IsCubeOpen() || ch->GetOfflineShopOwner() || ch->IsAcceOpen())
	{
